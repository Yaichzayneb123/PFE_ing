...
import { Commande, CommandeF } from 'src/app/demo/modéle/commande';
...
export class MessagesDemoComponent {
  ...
  list:any
   ...
   // productss: { name: string, quantity: number, price: number, idDepot:number}[] = [];
    ...
    dropDownProduct: any;
    constructor(private productService: ProductService,private  Service: LayoutService,private messageService: MessageService,private commandeService:CommandeService,private fourService:FournisseurService ){}
    ...
    async ngOnInit() {
        ...
        this.clients = await this.fourService.getFourByIdsoc(this.currentUser.id);
...
console.log(this.totalAllProducts)
  ...
  this.commande.listcmdDto = selectedProductIds;
        ...
        const commande: CommandeF = {
          idFour: this.selectedClient?.id,
          produitList: [],
      ...
      console.log(commande);
        this.commandeService.addCommandeFour(commande).subscribe(
          (res) => {
